const UserNameChangePanel = (props) => {
    ...
    function changePreferredName (event) {
        ...
        const preferredName = document.getElementsByClassName("changeNameInput")[0].value;
        console.log(`user wants ${preferredName} and sessionid is ${props.sessionId}`);
        const sessionId = props.sessionId;

        // /updateusername?sessionid={sessionid}&pref={newname}

        fetch("http://localhost:8080/updateusername?sessionid="+sessionId+"&pref="+preferredName, {
            method: "POST",
            // mode: "no-cors",
            headers: {
        ...
        <div className = "changeNameDiv">
            <span className = "changeNameSpan">
                <form onSubmit={changePreferredName}> 
                    <input className = "changeNameInput" placeholder = "Enter new preferred name" required></input>
